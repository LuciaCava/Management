...
print(np.mean(all_delayCloud))
print(len(all_delayCloud))
print(np.mean(queuingTot)) 
print(len(queuingTot))
...
j=0 
x = np.average(avg_delayCloud)
x_k = []
r_k = []
n = len(avg_delayCloud)

for j in range(500):
    new_x = avg_delayCloud[j+1:]
...
'''
fog_servTime = np.arange(5, 10, 0.5)
avg_queuing = [] 
np.x = T_q 
