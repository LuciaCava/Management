...
'''
def checkServer(serv_num):
    bus = 'BusyServer' + str(serv_num)
            ...
            if assign == 1:
                while s1<4:
                    ...
                    if BusyServer[server]==False:
                        ...
                        env.process(departure_process(env, service_time,queue,serv,server))
                        s1=4
                    s1=s1+1
            elif assign == 2:
                server_list = [0,1,2,3]
...
def departure_process(environment, service_time,queue, serv,i):
    ...
    if len(queue)<=4: 
        ...
        busy_time[i] = busy_time[i] + service_time
...
print(f"and was busy the {(busy_time[1]/env.now)*100} % of the time")

print("\nThe third server processed packets: ")
print(f"and was busy the {(busy_time[2]/env.now)*100} % of the time")

print("\nThe fourth server processed packets: ", data2.arr)
print(f"and was busy the {(busy_time[3]/env.now)*100} % of the time")
print('-----------------------------------------------------')
...
'''
Non ha senso plottare grafici del busy time, secondo me in questo caso è meglio nel report aggiungere una tabella
con riportate le diverse busy_time e come l'operational cost diminuisci utilizzando il metodo di assegnazione 'LEAST COSTLY' 
'''


''' 
plt.plot(loads, perc_serv2[0,:], 'r', label='Round Robin')
plt.plot(loads, perc_serv2[1,:], 'b',label='Random')    
plt.plot(loads, perc_serv2[2,:], 'g', label='Least Costly') 
 
plt.xlabel('Loads')
plt.ylabel('% busy time')
plt.title('% busy time for server 2')
plt.legend()
plt.show()
